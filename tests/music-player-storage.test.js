...
describe('MusicPlayerStorage', () => {
    ...
    it('ensureDefaultPlaylists renames legacy 2025 beyond afterlife id to 2026', () => {
        const storage = new window.MusicPlayerStorage();
        const data = {
            playlists: [
                {
                    ...
                    songs: [{ id: 'x', title: 'kept' }]
                }
            ],
            currentPlaylistId: '2025 beyond afterlife'
        };
        storage.ensureDefaultPlaylists(data);
        expect(data.playlists.some(p => p.id === '2025 beyond afterlife')).toBe(false);
        const p = data.playlists.find(pl => pl.id === '2026 beyond afterlife');
        expect(p).toBeDefined();
        expect(p.name).toBe('2026 beyond afterlife');
        expect(p.songs.some(s => s.id === 'x')).toBe(true);
        expect(data.currentPlaylistId).toBe('2026 beyond afterlife');
    });
});
